...
namespace AdangalApp
{
    ...
    public static void DeleteAdangalFile(Adangal adangalToDelete)
    {
        var data = ReadFileAsObjects<Adangal>(JsonPath);

        var itemToDelete = data.Where(w => w.NilaAlavaiEn == adangalToDelete.NilaAlavaiEn &&
                        w.UtpirivuEn == adangalToDelete.UtpirivuEn &&
                        w.PattaEn == adangalToDelete.PattaEn &&
                        w.LandStatus == adangalToDelete.LandStatus).FirstOrDefault();
